import { CalculationResult, TaxBand, TaxStrategy, PITResult, TaxBandResult } from './app.types';
...
export function calculateFutureBalance(
  ...
  const futureValueSeries = r > 0
    ? PMT * ((Math.pow(1 + r, n * t) - 1) / r)
    : PMT * n * t;
  ...
  if (taxStrategy === 'wht') {
    ...
    const taxWithInterest = calculateProgressiveTax(annualIncome + totalInterest, NIGERIA_PIT_BANDS_2026);
