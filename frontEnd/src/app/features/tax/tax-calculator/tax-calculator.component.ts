...
export class TaxCalculatorComponent implements OnInit {
	// --- Services ---
	private readonly taxApiService = inject(TaxApiService);
	...
	ngOnInit(): void {
		...
		const scenario = nav?.extras?.state?.['scenario'] as
			| import('@app/shared/types/scenario.types').ScenarioResponse
			| undefined;
	...
	protected onSaveToScenarios(): void {
		this.scenarioApi
			.save({
				name: `Tax Plan - ₦${this.annualIncome().toLocaleString()}`,
				principal: 0,
				annualRate: 0,
				years: 1,
			...
			.subscribe({
				next: () => alert('Tax calculation saved to your scenarios!'),
				error: () => alert('Failed to save tax plan.'),
			});
	}

	protected onDownloadReport(): void {
		this.taxApiService.onDownloadReport(this.annualIncome()).subscribe((blob) => {
			const url = window.URL.createObjectURL(blob);
			const link = document.createElement('a');
			link.href = url;
			link.download = `tax_report_${Date.now()}.pdf`;
			link.click();
		});
	}
}
